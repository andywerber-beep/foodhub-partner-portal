...
export const ActiveDashboardView: React.FC = () => {
  ...
  const venueName = partner?.name || 'Partner Venue';
  ...
  useEffect(() => {
    const checkOffers = async () => {
      ...
      try {
        ...
        console.error('Error evaluating real-time offer map matrix:', err);
      }
    };
          ...
          <div style={{ position: 'relative', width: '100%', height: '380px', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px', border: '1px solid #262626' }}>
            ...
            {isHovered && (
              <div style={{
                ...
                left: '20px',
                background: '#1a1a1a',
                border: hasActiveOffers ? '1px solid #4CD137' : '1px solid #333',
                padding: '16px',
                borderRadius: '12px',
                color: '#fff',
                width: '260px',
                ...
                <h4 style={{ margin: '0 0 4px 0', fontSize: '15px', fontWeight: 700, color: '#fff' }}>{venueName}</h4>
        ...
        <div style={{ textAlign: 'left', background: '#141414', padding: '32px', borderRadius: '16px', border: '1px solid #222' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
            <div style={{ backgroundColor: '#1a1a1a', padding: '24px', borderRadius: '12px', border: '1px solid #262626' }}>
              <span style={{ fontSize: '11px', color: '#666', fontWeight: 700, display: 'block', marginBottom: '8px', letterSpacing: '0.5px' }}>DAILY REVENUE</span>
              <h4 style={{ fontSize: '32px', color: '#ffffff', margin: 0, fontWeight: 800 }}>£{metrics.dailyRevenue}</h4>
            </div>
            <div style={{ backgroundColor: '#1a1a1a', padding: '24px', borderRadius: '12px', border: '1px solid #262626' }}>
              <span style={{ fontSize: '11px', color: '#666', fontWeight: 700, display: 'block', marginBottom: '8px', letterSpacing: '0.5px' }}>MONTHLY REVENUE</span>
              <h4 style={{ fontSize: '32px', color: '#ffffff', margin: 0, fontWeight: 800 }}>£{metrics.monthlyRevenue}</h4>
            </div>
            <div style={{ backgroundColor: '#1a1a1a', padding: '24px', borderRadius: '12px', border: '1px solid #262626' }}>
              <span style={{ fontSize: '11px', color: '#666', fontWeight: 700, display: 'block', marginBottom: '8px', letterSpacing: '0.5px' }}>CUSTOMER FOOTFALL</span>
              <h4 style={{ fontSize: '32px', color: '#ffffff', margin: 0, fontWeight: 800 }}>{metrics.footfall} <span style={{ fontSize: '14px', color: '#444', fontWeight: 500 }}>visits</span></h4>
            </div>
          </div>
        </div>
      )}
